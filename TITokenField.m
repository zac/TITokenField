...
- (CGFloat)layoutTokensInternal {
	...
	[_tokens enumerateObjectsUsingBlock:^(TIToken * token, NSUInteger idx, BOOL *stop){
		...
		if (token.superview){
			...
			_tokenCaret.x += token.bounds.size.width;
...
- (instancetype)initWithTitle:(NSString *)aTitle representedObject:(id)object font:(UIFont *)aFont {
	...
	if ((self = [super init])){
		...
		self.font = aFont;
        self.tintColor = [TIToken blueTintColor];
...
- (void)setHighlighted:(BOOL)flag {
	...
	if (self.highlighted != flag){
        ...
        if (self.highlighted) {
            ...
            self.titleLabel.highlighted = flag;
...
- (void)layoutSubviews
{
    ...
    // lay out label.
    CGSize titleSize = [_title sizeWithFont:_font forWidth:(_maxWidth - hTextPadding) lineBreakMode:kLineBreakMode];
    ...
    self.titleLabel.frame = textBounds;
}
